...
data_long <- autism %>%                            # save the dataset as a new name
  ...
  dplyr::mutate(age = 2 + age2) %>%                # create the original age variable 
...
data_wide <- data_long %>%           # save the dataset as a new name
  dplyr::select(-age) %>%            # delete (by deselecting) this variable 
...
It is nice that the `stat_summary()` layer computes the standard error for the mean for you using the data in LONG format, but I can't figure out how to connect the lines.
...
data_long %>% 
  ggplot(aes(x = age,
             y = vsae,
             color = sicdegp)) +
  stat_summary() +                         # default = means and standard error for the mean
  theme_bw() +
  scale_x_continuous(breaks = c(2, 3, 5, 9, 13)) +
...
data_sum_all %>% 
  ...
  aes(x = age,
      y = vsae_mean,
      color = sicdegp) +
  geom_errorbar(aes(ymin = vsae_mean - vsae_sem,
