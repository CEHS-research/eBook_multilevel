...
data_achieve <- data_raw %>% 
  dplyr::mutate_at(vars(id, region, corp, school, class),    # declare these are factors
                   factor) %>% 
  dplyr::mutate(gender = gender %>% 
                  factor(labels = c("Female", "Male"))) %>%  # guess based on subset sizes
  dplyr::mutate(classize = classize %>% 
                  factor(labels = c("12-17", "18-21",        # guess based on subset sizes
                                    "22-26", ">26"))) %>% 
...
*Note: I had to manually install the `hexbin` package  for the `geom_hex()` to run.*
...
data_achieve %>% 
  ...
  geom_smooth(method = "lm",                              # Single-level Regression Line
              SE = FALSE) +                               #  (without the error bands)
  theme_bw()
```
...
texreg::screenreg(list(fit_read_0ml, fit_read_1ml),
                  custom.model.names = c("Null", "w L1vPred"),
                  digits = 3)
```
...
```{r, include = FALSE}
screenreg(list(fit_read_3ml, fit_read_6ml, fit_read_7ml, fit_read_8ml),
          custom.model.names = c("Level 1 only", "Both Levels", "Cross-Level", "3-way"),
          digits = 4)
```
...
```{r, include=FALSE}
screenreg(list(fit_read_9ml, fit_read_10ml, fit_read_9ml_c, fit_read_10ml_c),
...
```{r, include = FALSE}
screenreg(list(fit_read_10ml_s, fit_read_11ml_s, fit_read_12ml_s),
          custom.model.names = c("no SES", "2-way", "3-way"),
          digits = 4)
```
