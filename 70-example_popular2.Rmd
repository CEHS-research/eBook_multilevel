...
@hox2017 labeled this as "$M_0$" in chapter 2:
...
> Note: the `VarCorr()` function in the `lme4` package returns the standard deviations, not the variances ($var = SD^2$) for a model fit via the `lme4::lmer()` function.  The `summary()` function reports both the variances and the stadard deviations.
...
### Adding Predictors to the Model: "M1" (page 17)

@hox2017 labeled this as "$M_1$" in chapter 2:
...
\begin{align*} 
class (intercept)   \longrightarrow \sigma^2_{u_0 }   & = var[u_{0j}] \\
pupil \; gender     \longrightarrow \sigma^2_{u_1}    & = var[u_{1j}] \\
pupil \; extravert  \longrightarrow \sigma^2_{u_2}    & = var[u_{2j}] \\
error -or- residual \longrightarrow \sigma^2_{e}      & = var[e_{ij}] 
\end{align*}
$$
