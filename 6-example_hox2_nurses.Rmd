...
```{r}
table(data_nurse$expconNG)
```

```{r}
table(data_nurse$hospsizeNG)
```
...
```{r, results='asis'}
texreg::htmlreg(list(nurse_lmer_0_re_2level, nurse_lmer_0_re),
                custom.model.names = c("2 levels", "3 levels"),
                single.row = TRUE)
```
...
```{r, results='asis'}
texreg::htmlreg(list(nurse_lmer_1d_re, nurse_lmer_2_re),
